...
def get_sp500():
    try:
        ...
        if "SPY" not in s: s.append("SPY")
        return sorted(list(dict.fromkeys(s)))
    except: return ["SPY","AAPL","MSFT","NVDA","META","AMZN","GOOGL","SPY"]
...
def get_irx_curve(start, end):
    ...
    c = r.get("Close", None)
    if c is None: c = r.get("Adj Close", None)
...
def compute(df, strike_pct=0.05, barrier_pct=0.10, tenor_days=90, contract_size=100):
    ...
    for i0, i1 in roll_windows_index(dates, tenor_days):
        ...
        out.append({"Start":d0,"End":d1,"S0":S0,"K":K,"B":B,"ST":ST,"KO":knocked,
...
c3,c4 = st.columns(2)
strike_pct = c3.number_input("Strike offset (e.g. 0.05=5%)",value=0.05,min_value=0.0,max_value=0.9,step=0.01,format="%.2f")
barrier_pct = c4.number_input("Barrier offset (e.g. 0.10=10%)",value=0.10,min_value=0.0,max_value=0.95,step=0.01,format="%.2f")
...
if hist.empty: st.error("No data."); st.stop()
res = compute(hist,strike_pct,barrier_pct,tenor_days,100)
if res.empty: st.warning("Not enough data."); st.stop()

irx = get_irx_curve(start,today)
cum = irx["cum"]; cum_T = float(cum.iloc[-1])
...
with left:
    ...
    base = alt.Chart(long_df).encode(
        ...
        color=alt.Color("Type:N",scale=alt.Scale(range=["#ff7f0e","#1f77b4"])),
...
with right:
    ...
    st.table(stats.style.format({"Mean":"{:.0f}","Median":"{:.0f}","Std":"{:.0f}","Hit rate (>0)":"{:.4f}"}))
    st.metric("KO freq (%)",f"{100*res['KO'].mean():.1f}%")

st.subheader("Recent Trades (last 10)")
show = res.tail(10).copy()
st.dataframe(show[["Start","S0","K","B","ST","KO","FV_OutPut_USD","FV_Vanilla_USD"]].style.format({
    "S0":"{:.2f}","K":"{:.2f}","B":"{:.2f}","ST":"{:.2f}",
    "FV_OutPut_USD":"{:.0f}","FV_Vanilla_USD":"{:.0f}"}),use_container_width=True)
